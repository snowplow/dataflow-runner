...
const (
	CLUSTER_SCHEMA_PATH  = "resources/avro/cluster.avsc"
	PLAYBOOK_SCHEMA_PATH = "resources/avro/playbook.avsc"
)
...
// ParseClusterRecordFromFile attempts to parse a JSON file to a ClusterRecord
func (cr ConfigResolver) ParseClusterRecordFromFile(filePath string, variables map[string]interface{}) (*ClusterRecord, error) {
...
// ParseClusterRecord attempts to parse a JSON file to a ClusterRecord
func (cr ConfigResolver) ParseClusterRecord(jsonBytes []byte, variables map[string]interface{}) (*ClusterRecord, error) {
	...
	decodedRecord := new(ClusterRecord)
...
// ParsePlaybookRecordFromFile attempts to parse a JSON file to a PlaybookRecord
func (cr ConfigResolver) ParsePlaybookRecordFromFile(filePath string, variables map[string]interface{}) (*PlaybookRecord, error) {
	jsonBytes, err := FilePathToByteArray(filePath)
	if err != nil {
		return nil, err
	}
...
// ParsePlaybookRecord attempts to parse a JSON file to a PlaybookRecord
func (cr ConfigResolver) ParsePlaybookRecord(jsonBytes []byte, variables map[string]interface{}) (*PlaybookRecord, error) {
	sdr, err := toSelfDescribingRecord(jsonBytes, variables)
	if err != nil {
		return nil, err
	}
	...
	// Write and decode record as Avro
	decodedRecord := new(PlaybookRecord)
