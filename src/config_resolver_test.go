...
var ClusterRecord_1 = `{
  "schema": "iglu:com.snowplowanalytics.dataflow-runner/Cluster/avro/1-0-0",
  "data": {
    "name": "xxx",
    "logUri": "s3://logging/",
    "region": "us-east-1",
...
func TestParseClusterRecord_Success(t *testing.T) {
	...
	res, err := ar.ParseClusterRecord([]byte(ClusterRecord_1), nil)
	...
	res, err = ar.ParseClusterRecord([]byte(ClusterRecord_2), nil)
...
func TestParsePlaybookRecord_Success(t *testing.T) {
	...
	ar, _ := InitConfigResolver()
	res, err := ar.ParsePlaybookRecord([]byte(PlaybookRecord_1), nil)
...
func TestTemplateRawBytes(t *testing.T) {
	...
	byteArr := []byte(`{"key":"{{systemEnv "TEST_ENV_VAR"}}","key2":"{{ .someVar}}"}`)
	templatedByteArr, err := templateRawBytes(byteArr, varMap)

	assert.NotNil(templatedByteArr)
	assert.Nil(err)
	assert.Equal(`{"key":"golangTestEnvVar","key2":"golangTestVar"}`, string(templatedByteArr))
}
