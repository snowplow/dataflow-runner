...
const (
	INVALID_STATE_SLEEP_SECONDS = 30
)
...
func InitEmrCluster(clusterRecord ClusterRecord) *EmrCluster {
	return &EmrCluster{
		Config: clusterRecord,
	}
}
...
func (ec EmrCluster) waitForState(svc *emr.EMR, jobflowId string, neededState string, exitStates []string) (*emr.ClusterStatus, error) {
	...
	for !StringInSlice(*resp.Cluster.Status.State, exitStates) {
		log.Info("EMR cluster is in state " + *resp.Cluster.Status.State + " - need state " + neededState + ", checking again in " + strconv.Itoa(INVALID_STATE_SLEEP_SECONDS) + " seconds...")

		time.Sleep(time.Second * INVALID_STATE_SLEEP_SECONDS)
...
func (ec EmrCluster) GetBootstrapActions() []*emr.BootstrapActionConfig {
	...
	var emrBootstrapActionArr []*emr.BootstrapActionConfig = nil
...
func (ec EmrCluster) GetConfigurations() []*emr.Configuration {
	...
	var emrConfigurationArr []*emr.Configuration = nil
