...
func TestConsulLock(t *testing.T) {
	...
	err = ocl.TryLock()
	assert.NotNil(err)
	assert.Equal("Lock currently held", err.Error())
...
func TestFileLock(t *testing.T) {
	...
	// doesn't work if the file already exists no matter its content
	err = ioutil.WriteFile(lockPath, []byte("something\n"), 0666)
	assert.Nil(err)
}
...
func TestGetLock(t *testing.T) {
	...
	assert.NotNil(lock)
	assert.Nil(err)
	assert.Equal(lock, &FileLock{lock: lockfile.Lockfile(lockName)})
