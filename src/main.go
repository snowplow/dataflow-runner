...
const (
	...
	CLI_VERSION    = "0.1.0-rc2"
	VAR_DELIM      = ","
	F_EMR_CONFIG   = "emr-config"
	F_EMR_PLAYBOOK = "emr-playbook"
	F_EMR_CLUSTER  = "emr-cluster"
	F_VARS         = "vars"
	F_ASYNC        = "async"
)
...
func main() {
	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Action: func(c *cli.Context) error {
				err := run(
					c.String(F_EMR_PLAYBOOK),
					c.String(F_EMR_CLUSTER),
					c.Bool(F_ASYNC),
					c.String(F_VARS),
				)
				checkErr(err)
		...
		},
		{
			...
			Action: func(c *cli.Context) error {
				emrConfig := c.String(F_EMR_CONFIG)
				emrPlaybook := c.String(F_EMR_PLAYBOOK)
				vars := c.String(F_VARS)
...
func getEmrConfigFlag() cli.StringFlag {
	return cli.StringFlag{Name: F_EMR_CONFIG, Usage: "EMR config path"}
}

func getEmrPlaybookFlag() cli.StringFlag {
	return cli.StringFlag{Name: F_EMR_PLAYBOOK, Usage: "Playbook path"}
}

func getEmrClusterFlag() cli.StringFlag {
	return cli.StringFlag{Name: F_EMR_CLUSTER, Usage: "Jobflow ID"}
}

func getVarsFlag() cli.StringFlag {
	return cli.StringFlag{Name: F_VARS, Usage: "Variables that will be used by the templater"}
}

func getAsyncFlag() cli.BoolFlag {
	return cli.BoolFlag{Name: F_ASYNC, Usage: "Asynchronous execution of the jobflow steps"}
}
...
func up(emrConfig string, vars string) (string, error) {
	if emrConfig == "" {
		return "", flagToError(F_EMR_CONFIG)
	}
...
func run(emrPlaybook string, emrCluster string, async bool, vars string) error {
	if emrPlaybook == "" {
		return flagToError(F_EMR_PLAYBOOK)
	}
	if emrCluster == "" {
		return flagToError(F_EMR_CLUSTER)
	}
...
func varsToMap(vars string) (map[string]interface{}, error) {
	...
	if len(varsArr)%2 != 0 {
		return nil, errors.New("--" + F_VARS + " must have an even number of keys and values")
	}
